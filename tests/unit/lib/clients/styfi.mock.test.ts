...
describe("MockStyfiClient", () => {
  ...
  it("auto-claims liquid funds when starting a new cooldown", async () => {
    const client = createMockStyfiClient({ latencyMs: 0 });
    const amount = 100n * 10n ** 18n;
    ...
    expect(state.styfiUnlocked).toBe(amount / 2n);
    expect(state.styfiInCooldown).toBe(amount / 2n + 10n * 10n ** 18n);
  });
});
