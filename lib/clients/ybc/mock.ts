import mockFixture from "@/docs/apps/ybc/examples/mock-data.example.json";
...
const ybcMockData = mockFixture as YbcMockExampleScenariosV1;
...
const scenarioMap = Object.fromEntries(
  ybcMockData.scenarios.map((scenario) => [scenario.id, scenario])
) as Record<YbcScenarioId, YbcMockScenario>;

export function listYbcMockScenarios(): readonly YbcMockScenario[] {
  return ybcMockData.scenarios;
}
...
export function cloneYbcMockScenarioData(id: YbcScenarioId): YbcMockDataV1 {
  return structuredClone(getYbcMockScenario(id).data);
}
...
export function getYbcProposalThresholdState(proposal: YbcProposalRecord) {
  ...
  const currentBps =
    total <= 0 ? 0 : Math.round((yea / total) * 10_000);
...
export function createYbcMockProposal(
  ...
  const createdAt = data.asOf + existingItems.filter((item) => item.phase === "discussion").length * 300;
