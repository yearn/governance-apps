...
its dependencies in full before assigning work. Verify that {M0_SPEC_COMMIT} is
an ancestor of agent/integration and that the annotated tag integration/dao-m0
resolves there. Stop and report the mismatch if either check fails. Do not redo
discovery unless repository code or a newer pinned contract revision
